...
public class EndersoulHandItem extends Item {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (!canCarry(worldIn, pos, blockState))
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        RayTraceResult result = rayTrace(playerIn, 128.0D);
        if (result == null || result.typeOfHit != RayTraceResult.Type.BLOCK) {
            playerIn.sendStatusMessage(new TextComponentTranslation("Unable to teleport to location"), true);
        ...
        if (!worldIn.isRemote) {
            ...
            playerIn.setPositionAndUpdate(endPos.getX() + 0.5D, endPos.getY(), endPos.getZ() + 0.5D);
